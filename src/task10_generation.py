...
def generate_with_citation(query: str, top_k: int = TOP_K) -> dict:
    ...
    user_message = (
        "Context below is the only evidence you may use. Cite every factual claim "
        "with the matching [Document N] label. If the evidence is insufficient, "
        f"return a short refusal.\n\nContext:\n{context}\n\nQuestion: {query}"
    )
    try:
        answer = call_llm(SYSTEM_PROMPT, user_message) or refusal
    except Exception:
        answer = refusal
    retrieval_source = (
